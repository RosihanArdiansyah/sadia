...
//error_reporting(E_ALL);			
				...
				while ($c = mysql_fetch_array($query))
				{
                    $isi_berita = htmlentities(strip_tags($c[fulltext_content])); // membuat paragraf pada isi berita dan mengabaikan tag html
						...
						<a title="<?php echo $c['title_content'];?>" href="buletin-<?=$c[id_content]?>-<?=$c[alias_content]?>.html" class="image-post item-overlay ">
							<img src="images/stories/<?php echo $c['images'];?>" alt="">
						</a>

						<header class="entry-header"><meta http-equiv="Content-Type" content="text/html; charset=utf-8">
							<h3 class="entry-title"><a href="buletin-<?=$c[id_content]?>-<?=$c[alias_content]?>.html"><?php echo $c['title_content'];?></a></h3>
						</header>
...
<hr />
		...
		<!--/ #main-->
			...
			<?php
						 ...
						 $jmldata     = mysql_num_rows(mysql_query("SELECT * FROM $berita WHERE catid = '42'"));
						  $jmlhalaman  = $p->jumlahHalaman($jmldata, $batas);
						  $linkHalaman = $p->navHalaman($_GET['halbuletin'], $jmlhalaman);
