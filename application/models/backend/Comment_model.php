...
class Comment_model extends CI_Model{
	...
	function get_all_comment($offset,$limit){
		$result = $this->db->query("SELECT comment_id,DATE_FORMAT(comment_date,'%d %M %Y %H:%i') AS comment_date,comment_name,comment_email,comment_status,comment_message,comment_image,post_id,post_title,post_slug FROM tbl_comment JOIN tbl_posts ON comment_post_id=post_id WHERE comment_parent='0' ORDER BY comment_id DESC LIMIT $offset,$limit");
		return $result;
	}

	function get_all_comment_unpublish($offset,$limit){
		$result = $this->db->query("SELECT comment_id,DATE_FORMAT(comment_date,'%d %M %Y %H:%i') AS comment_date,comment_name,comment_email,comment_status,comment_message,comment_image,post_id,post_title,post_slug FROM tbl_comment JOIN tbl_posts ON comment_post_id=post_id WHERE comment_status='0' ORDER BY comment_id DESC LIMIT $offset,$limit");
		return $result;
	}
	...
	function search_comment($keyword){
		$hsl=$this->db->query("SELECT comment_id,DATE_FORMAT(comment_date,'%d %M %Y %H:%i') AS comment_date,
			...
			post_id,post_title,post_slug FROM tbl_comment LEFT JOIN tbl_posts ON comment_post_id=post_id 
